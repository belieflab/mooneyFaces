...
let full_stim = [];
  for (let i = 0; i < original.length ; i++){
    full_stim.push({stimulus: original_stimuli[i], data: {stimulus: original[i], stim_id: original[i]+".jpg", test_part: 'original', correct_response: '1', incorrect_response: '0'}},)
  }
  for (let i = 0; i < inverted.length ; i++){
    full_stim.push({stimulus: inverted_stimuli[i], data: {stimulus: inverted[i], stim_id: inverted[i]+".jpg", test_part: 'inverted', correct_response: '0', incorrect_response: '1'}},)
  }
